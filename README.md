...
Прод-версия research-ноутбука `notebooks/AML_mule_detection_fixed.ipynb`:
детекция дропперов (money mules) для отдела финансового мониторинга.
Ноутбук остаётся источником правды по EDA и обоснованию решений; этот
репозиторий -- воспроизводимая обёртка вокруг того же пайплайна.
...
- `Is_Mule` восстанавливается из префикса `Customer_ID`; строки без ID
  удаляются, а не размечаются как "не дроппер" -- иначе метка тихо портится.
- Импутер/скейлер обучаются только на train (`Preprocessor.fit` вызывается
  один раз, `transform` -- на val/test/проде).
...
- Метрика выбора модели -- PR-AUC (Accuracy не годится при ~5% дропперов).
  Порог подбирается по F2 (recall важнее precision, но не бесконечно).
- В ноутбуке (раздел 11) отдельно доказано: слабость на паре MULE/TRADER --
  свойство датасета, а не конкретного алгоритма (проверено на 4 разных
  моделях).
...
сам сгенерирует синтетические данные (если не смонтирован реальный
`data/data.csv`) и обучится при первом запуске -- см. `entrypoint.sh`.
Для реального использования смонтируйте свой `data/data.csv`.
...
## Известное ограничение модели (см. ноутбук, разделы 10--11)

Решение почти целиком опирается на `Account_Age_Days` и
`Avg_Time_Between_Trans_Min`. Дроппер, имитирующий "состаренный" счёт и
нормальную скорость операций, может обмануть `primary_model` -- на этот
случай и добавлен `fallback_model`, но раздел 11 ноутбука честно
показывает, что резервный сигнал в оставшихся признаках слабый почти для
любого алгоритма. Это ограничение данных, а не конкретной модели, и его
стоит явно проговаривать, а не скрывать за общими метриками.
